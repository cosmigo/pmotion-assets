...
Although I've started working on this document in March 2019, it's still far for complete -- and, possibly, far from being usable in real case scenarios, except for very simple plug-in types.
...
The _Developer Interface_ document unfortunately doesn't cover in depth the various contexts in which the DLL functions are invoked, or how different images or animations might affect their parameters, leaving many aspects of the interface rather obscure.
...
In the last year I've started to develop a variety of plug-ins, but soon stumbled into dead ends with each one of them, due to undocumented aspects of the DLL interface behaviour.
...
Unfortunately, I've reached a point in which no further investigation of the interface is possible, unless it's official documentation is updated.
Further attempts to deduce the contextual behaviour of the interface via trail and error are too time-consuming and unlikely to improve the current situation.
Therefore, right now the chances that this document will ever be completed are rather slim.

Although the DLL plug-in interface was introduced over ten years ago, with Pro Motion 6.5 (around 2009), there still aren't any plug-ins available except for the simImage and sanAnimation sample plug-ins.
It's difficult to say if no further energy was invested in the plug-in documentation due to lack of end users interests, or whether the lack of user-created File I/O plug-ins is due to incomplete documentation.
Having invested so many hours on the DLL interface in the past year, I tend to believe that the latter is the cause (after all, various DDE plug-ins were contributed by end users, but no DLL plug-ins).

It's a pity, because the File I/O interface had a great potential for Pro Motion, allowing users to extend the application to support new graphic formats (a much desirable feature, especially for game developers targeting modern game engines).
Hopefully, in the future the official documentation for the File I/O interface will be updated and expanded on; in the meantime I'm leaving behind these notes, in the hope that they might be of help to those wishing to dive into plug-ins development.
